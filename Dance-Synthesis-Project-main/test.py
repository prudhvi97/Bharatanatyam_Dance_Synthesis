...
random_select = False
static_begin_frame = 90
static_test_file = 0
test_interval = [120, 120, 90]
vel_factor_change_ratio = 1.0

# change velocity factor
vel_factor_choose = True
vel_factor_begin_frame = 90
vel_factor_file = 0

# change first key frame data
first_keyframe_choose = True
first_keyframe_frame = 90
first_keyframe_file = 0

# change last key frame data
last_keyframe_choose = True
last_keyframe_frame = 260  # 66
last_keyframe_file = 10 # 122

# change all keyframe pose
all_keyframe_choose = True
all_keyframe_begin_frame = 90
all_keyframe_file = 0

# KeyFrames from multiple files
all_keyframe_choose_multiple = True
key_frame_modifications = [
    {"test_file": 2, "key_frame": 134},
    {"test_file": 10, "key_frame": 295}
]
...
def modify_intermediate_key_frames(data_set, key_frame_modifications, test_data, target, mask, config):
    print("*********INTO Modify intermediate key frames*************")
    key_frame_positions = [i for i, x in enumerate(mask) if x == 1]
    # Exclude the first and last key frame positions for start and end frames, handled separately
    intermediate_positions = key_frame_positions[1:-1]

    target_idx = 0
    for pos_idx, modification in zip(intermediate_positions, key_frame_modifications):
        file_idx = modification["test_file"]
        frame_idx = modification["key_frame"]
        key_frame_data = data_set[file_idx][frame_idx]
        print(f"frame {frame_idx} used from {file_idx} for position keyframe {pos_idx}")
        # Insert the key frame data into test_data and target at the specified positions
        insert_key_frame_data(test_data, target, pos_idx, key_frame_data, config,target_idx)
        target_idx += 1

def insert_key_frame_data(test_data, target, pos_idx, key_frame_data, config,i):
    test_data[0, pos_idx, :config.pos_dim] = key_frame_data[:config.pos_dim]
    loc = config.pos_dim + config.root_pos_dim
    test_data[0, pos_idx, loc:loc + config.root_rot_dim] = key_frame_data[loc:loc + config.root_rot_dim]
    target[0, i+1, :config.pos_dim] = key_frame_data[:config.pos_dim]
    target[0, i+1, loc:loc + config.root_rot_dim] = key_frame_data[loc:loc + config.root_rot_dim] 
...
def test_prediction_network(args):
    ...
    if all_keyframe_choose_multiple:
        print("**********INTO all keyframes choose multiple*************")
        test_section["all_keyframe_choose_multiple"] = all_keyframe_choose_multiple
        test_section["all_keyframe_multiple_frames"] = key_frame_modifications
        modify_intermediate_key_frames(data_set, key_frame_modifications, test_data, target, mask, config)
